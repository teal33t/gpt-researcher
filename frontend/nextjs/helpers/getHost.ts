...
export const getHost = ({ purpose }: GetHostParams = {}): string => {
  if (typeof window !== 'undefined') {
    ...
    if (apiUrlInLocalStorage) {
    ...
    } else if (process.env.NEXT_PUBLIC_GPTR_API_URL && process.env.NEXT_PUBLIC_GPTR_API_URL !== '/') {
      ...
      return host.includes('localhost') ? 'http://localhost:8001' : `https://${host}`;
    }
  }
  // Fallback for server-side rendering during build
  return process.env.NEXT_PUBLIC_GPTR_API_URL && process.env.NEXT_PUBLIC_GPTR_API_URL !== '/' 
